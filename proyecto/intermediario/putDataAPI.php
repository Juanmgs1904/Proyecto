...
if (isset($_POST['codigo']) && isset($_POST['peso']) && isset($_POST['estado']) && isset($_POST['fRecibo']) && isset($_POST['fEntrega']) && isset($_POST['destinatario']) && isset($_POST['destino'])) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
    curl_setopt($ch, CURLOPT_URL, "localhost/proyecto/Controller/almacen/C_paquetes.php");
    curl_setopt($ch, CURLOPT_POSTFIELDS, $json);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_exec($ch);
    if (curl_errno($ch)) {
...
if (isset($_POST['codigoE'])) {
    ...
    $json = json_encode($datos);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
    curl_setopt($ch, CURLOPT_URL, "localhost/proyecto/Controller/transito/C_paquetesE.php");
    curl_setopt($ch, CURLOPT_POSTFIELDS, $json);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_exec($ch);
    if (curl_errno($ch)) {
...
if (isset($_POST['IDLE'])) {
    ...
    $json = json_encode($datos);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
    curl_setopt($ch, CURLOPT_URL, "localhost/proyecto/Controller/transito/C_lotesE.php");
    curl_setopt($ch, CURLOPT_POSTFIELDS, $json);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_exec($ch);
    if (curl_errno($ch)) {
...
if(isset($_POST['motivo'])){
    ...
    if($_POST['motivo'] != null){
        ...
        $json = json_encode($datos);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
        curl_setopt($ch,CURLOPT_URL,"localhost/proyecto/Controller/transito/C_conduce.php");
        curl_setopt($ch, CURLOPT_POSTFIELDS, $json);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_exec($ch);
        if(curl_errno($ch)){
...
if(isset($_GET['matricula']) && isset($_GET['disponibilidad'])){
    ...
    $json = json_encode($datos);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
    curl_setopt($ch,CURLOPT_URL,"localhost/proyecto/Controller/almacen/C_camiones.php");
    curl_setopt($ch, CURLOPT_POSTFIELDS, $json);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_exec($ch);
    if(curl_errno($ch)){
